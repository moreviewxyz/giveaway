...
if ( $id ){
        ...
        if ($_id[2] != null){
                ...
                //if ($row3['images']['stills']!=null) {
                        //shuffle($row3['images']['stills']);
                        //foreach($row3['images']['stills'] as $result) {
                                //$images = 'https://image.tmdb.org/t/p/w780' . $result['file_path'];
                        //}
                //} else {
                ...
                if (is_array($row3['credits'][cast])){
                        ...
                        foreach($row3['credits'][cast] as $result) {
                                $casts[] = '<span itemprop="actor" itemscope itemtype="https://schema.org/Person"><span itemprop="name">' . $result['name'] . ' (' . $result['character'] . ')</span></span>, ';             
                        }
                        $cast = implode( ", ",$casts );
                }
                //if (is_array($row3['guest_stars'])){
                        ...
                        //foreach($row3['guest_stars'] as $result) {
                                //$guest[] = '<span itemprop="actor" itemscope itemtype="https://schema.org/Person"><span itemprop="name">' . $result['name'] . ' (' . $result['character'] . ')</span></span>';             
                        //}
                ...
                //if ($row2[episodes][$_id[1]]['still_path']!=null) {
                        //$images = 'https://image.tmdb.org/t/p/w780' . $row2['episodes'][$_id[1]][still_path];
                //} else {
                        //$images = '/include/images/no-backdrop.png';
                //}

                //if ($row2['images']['posters']!=null){
                        //shuffle($row2['images']['posters']);
                        //foreach($row2['images']['posters'] as $result) {
                                //$images_small = 'https://image.tmdb.org/t/p/w185' . $result['file_path'];
                        //}
                //} else {
                        //$images_small = '/include/images/no-poster-w185.jpg';
                //}
